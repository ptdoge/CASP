...
def soft_cross_entropy(input, target, weight=None, reduction='mean',ignore_index=255, alpha=0.1):
    """
    logSoftmax_with_loss
    :param input: torch.Tensor, N*C*H*W
    :param target: torch.Tensor, N*1*H*W,/ N*H*W
    ...
    target = target.long()
    if target.dim() == 4:
        target = torch.squeeze(target, dim=1) # 这里进行了一系列的处理
    if input.shape[-1] != target.shape[-1]:
        input = F.interpolate(input, size=target.shape[1:], mode='bilinear',align_corners=True)
    target = ((target == 0).long() * alpha) + target 

    return nn.CrossEntropyLoss(ignore_index=ignore_index, weight=weight, reduction=reduction).cuda()(input=input, target=target)
